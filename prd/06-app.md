...
当前阶段先提供临时测试页，用于验证登录后 `/app` 可访问、服务端 session 可读取，以及退出登录流程可用。完整工作台聚合能力后续再按本 PRD 扩展。
...
- 查看当前账号状态。
- 点击退出登录按钮，调用 Better Auth 退出当前 session，并跳转 `/sign-in`。
- 切换活跃组织。
- 进入组织、成员、安全、会话等页面。
- 接受或拒绝邀请。
...
- 临时测试页先展示当前用户名称、邮箱、邮箱验证状态、用户 ID，以及退出登录按钮。
- 无组织时展示创建组织引导。
- 邮箱未验证时在页面顶部展示提醒条。
