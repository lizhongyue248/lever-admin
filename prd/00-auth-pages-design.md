...
- 左侧品牌区的产品标识使用全局共享 `BrandLogo`，图形采用 Identity Mesh SVG；亮色主题使用 `public/logo-light.svg`，暗色主题使用 `public/logo-dark.svg`，不得再使用单字母占位标记。
...
- 卡片主标题使用页面级 `h1`，视觉上保持卡片标题样式，同时保证 Playwright 和辅助技术可以通过 heading 语义定位。
...
- 浏览器 favicon 使用同一套 Identity Mesh 品牌图形，并通过 `prefers-color-scheme` 分别提供亮色与暗色 SVG 资源。
